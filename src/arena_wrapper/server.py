...
Handles all communication with Unity through a Flask service.  Messages are sent to the controller
using a pair of request/response queues.
"""
...
from arena_wrapper.util.depth import apply_real_noise, generate_noise_indices
...
class Event:
    ...
    This class wraps the screenshot that Unity captures as well as the metadata sent about each
    object
    """
...
class Server:
    ...
    def create_event(self, metadata, files):
        if metadata["sequenceId"] != self.sequence_id:
            raise ValueError(
                "Sequence id mismatch: {} vs {}".format(metadata["sequenceId"], self.sequence_id)
            )
